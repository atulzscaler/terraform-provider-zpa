...
resource "zscaler_application_server" "sales" {
...
resource "zscaler_application_server" "intranet" {
...
resource "zscaler_application_server" "qa" {
...
resource "zscaler_application_server" "jenkins" {
...
resource "zscaler_application_server" "pan220" {
...
resource "zscaler_application_server" "vcenter" {
...
resource "zscaler_application_server" "cahlesx01" {
...
resource "zscaler_application_server" "cahlesx02" {
...
resource "zscaler_application_server" "trafficgen" {
...
resource "zscaler_application_server" "zpa131" {
...
resource "zscaler_application_server" "splunk" {
...
resource "zscaler_application_server" "nss128" {
...
resource "zscaler_application_server" "rdp125" {
...
resource "zscaler_application_server" "rdp126" {
...
resource "zscaler_application_server" "vcd125-ad01" {
...
data "zscaler_app_connector_group" "sgio-vancouver" {
  name = "SGIO-Vancouver"
}

// Server Groups
resource "zscaler_server_group" "all_other_services" {
  name = "all_other_services"
  description = "All Other Services"
  enabled = true
  dynamicdiscovery = true
  appconnectorgroups {
    id = [data.zscaler_app_connector_group.sgio-vancouver.id]
  }
}

resource "zscaler_server_group" "sgio_devops_servers" {
  ...
  dynamicdiscovery = false
    servers {
      id = [
        zscaler_application_server.jenkins.id
      ]
  }
    appconnectorgroups {
    id = [
      data.zscaler_app_connector_group.sgio-vancouver.id
    ]
  }
}

resource "zscaler_server_group" "sgio_intranet_web_apps" {
  ...
  dynamicdiscovery = false
    servers {
      id = [
        zscaler_application_server.intranet.id
        zscaler_application_server.qa.id
        zscaler_application_server.sales.id
      ]
    }
    appconnectorgroups {
    id = [
...
resource "zscaler_server_group" "sgio_monitoring_tools" {
  ...
  dynamicdiscovery = false
    servers {
      id = [
        zscaler_application_server.splunk.id
      ]
    }
    appconnectorgroups {
    id = [
...
resource "zscaler_server_group" "sgio_rdp_servers" {
  ...
  dynamicdiscovery = false
    servers {
      id = [
        zscaler_application_server.rdp125.id
        zscaler_application_server.rdp126.id
      ]
    }
    appconnectorgroups {
    id = [
...
resource "zscaler_server_group" "sgio_ssh_access" {
  ...
  dynamicdiscovery = false
    servers {
      id = [
        zscaler_application_server.splunk.id
        zscaler_application_server.pan220.id
        zscaler_application_server.trafficgen.id
        zscaler_application_server.nss128.id
      ]
    }
    appconnectorgroups {
    id = [
...
resource "zscaler_server_group" "sgio_vcenter_server" {
  ...
  dynamicdiscovery = false
    servers {
      id = [
        zscaler_application_server.vcenter.id
        zscaler_application_server.cahlesx01.id
        zscaler_application_server.cahlesx02.id
      ]
    }
    appconnectorgroups {
    id = [
...
resource "zscaler_segment_group" "all_other_services" {
  name = "all_other_services"
  description = "All Other Services"
  enabled = true
  policymigrated = true,
  applications {
    id = [
      zscaler_application_segment.all_other_services.id
    ]
  }
}

resource "zscaler_segment_group" "sgio_browser_access_apps" {
  ...
  applications {
    id = [
      zscaler_application_segment.sgio_browser_access_apps.id
    ]
  }
}

resource "zscaler_segment_group" "sgio_crm_apps_group" {
...
resource "zscaler_segment_group" "sgio_devops_servers" {
  name = "sgio_devops_servers"
  description = "SGIO DevOps Servers"
  enabled = true
  policymigrated = true,
...
resource "zscaler_segment_group" "sgio_domain_controllers" {
  ...
  applications {
    id = [
      zscaler_application_segment.sgio_domain_controllers.id
    ]
  }
}

resource "zscaler_segment_group" "sgio_intranet_web_apps" {
  name = "sgio_intranet_web_apps"
  description = "SGIO Intranet Web Apps"
  enabled = true
  policymigrated = true,
  applications {
    id = [
      zscaler_application_segment.sgio_intranet_web_apps.id
    ]
  }
}

resource "zscaler_segment_group" "sgio_monitoring_tools" {
  name = "sgio_monitoring_tools"
  description = "SGIO Monitoring Tools"
  enabled = true
  policymigrated = true,
  applications {
    id = [
      zscaler_application_segment.sgio_monitoring_tools.id
    ]
  }
}

resource "zscaler_segment_group" "sgio_rdp_services_group" {
  ...
  applications {
    id = [
      zscaler_application_segment.sgio_rdp_services_group.id
    ]
  }
}

resource "zscaler_segment_group" "sgio_ssh_access" {
  name = "sgio_ssh_access"
  description = "SGIO SSH Access"
  enabled = true
  policymigrated = true,
  applications {
    id = [
      zscaler_application_segment.sgio_ssh_access.id
    ]
  }
}
...
resource "zscaler_segment_group" "sgio_vcenter_server" {
  name = "sgio_vcenter_server"
  description = "SGIO vCenter Server"
  enabled = true
  policymigrated = true,
  applications {
    id = [
      zscaler_application_segment.sgio_vcenter_server.id
    ]
  }
}
...
output "zpa_segment_group" {
  value = zscaler_segment_group.zpa
}

// Testing Data Source Segment Group
data "zscaler_segment_group" "example" {
  id = 216196257331282105
}

output "all_segment_group" {
  value = data.zscaler_segment_group.example
}
...
output "all_app_connector_group" {
  value = data.zscaler_app_connector_group.example
}
...
output "all_server_group" {
  value = data.zscaler_server_group.example
}
...
output "all_posture_profile" {
  value = data.zscaler_posture_profile.example
}

// Testing Data Source Trusted Network 
data "zscaler_trusted_network" "example" {
}

output "all_trusted_network" {
  value = data.zscaler_trusted_network.example
}
...
output "idp_controller" {
    value = data.zscaler_idp_controller.all
}

// Testing Data Source Saml Attributes
data "zscaler_saml_attribute" "example" {
}

output "all_saml_attribute" {
  value = data.zscaler_saml_attribute.example
}
...
output "application_segment" {
    value = data.zscaler_application_segment.all
}